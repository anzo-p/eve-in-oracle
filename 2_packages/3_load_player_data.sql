...
*/
  ...
  t_assets                       CONSTANT t_char_corp := t_char_corp(
    ...
    --         name          id          char_corp   keyid      verification_code
...
--- STORE its data into a relational db format
      INSERT INTO tmp_load_assets -- GLOBAL TEMPORARY 
        ...
        AND   (   prt.material_origin  IN ('ICE', 'MINERAL', 'PLANET', 'MOON')
               OR prt.class            IN ('DATACORE', 'DECRYPTOR')
               OR prt.class          LIKE  'COMPONENT%');
...
*/
           ...
           LEFT OUTER JOIN (SELECT     eveapi_item_type_id
                            ...
                            FROM  (SELECT eveapi_item_type_id, quantity -- from Factory Hangars
                                   ...
                                   FROM   tmp_load_assets ast
                                   WHERE  ast.name                =  your characters name
